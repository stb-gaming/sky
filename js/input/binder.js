...
function collectInput(device, action, value) {
	...
	if (!devices[device].hasOwnProperty(action))
		devices[device][action] = { times: 0 };
...
function addKeyboardEvents() {
	["keyup", "keydown"].forEach((type) =>
		window.addEventListener(type, keyboardCollector, null, "sky"),
	);
}

function removeKeyboardEvents() {
	["keyup", "keydown"].forEach((type) =>
		window.removeEventListener(type, keyboardCollector, null, "sky"),
	);
}
...
function gamepadLoop() {
	...
	for (const gid in gamepads) {
		...
		const gamepad = gamepads[gid],
			lastGamepad = lastGamepads[gid] || {};
		if (gamepad.buttons) {
		...
		for (const bid in gamepad.buttons) {
			const button = gamepad.buttons[bid],
				lastButton = lastGamepad.buttons[bid] || false;
		...
		for (const aid in gamepad.axes) {
			const axis = gamepad.axes[aid],
				lastAxis = lastGamepad.axes[aid] || 0;
			if (lastAxis !== axis && Math.abs(axis) > 0.25) {
...
function createPopup(device, button) {
	...
	notice.textContent =
		"Pressing system buttons such as the home or back button will not work - this has been tested.";
...
async function bindInput(device, button) {
	...
	while (!bindingConfirmed) {
		...
		if (input.device == input2.device && input.action == input2.action) {
			if (!btnBindings.hasOwnProperty(input.device))
				btnBindings[input.device] = {};
...
function getButtonsFromInput(input) {
	...
	for (const skyButton in bindings) {
		...
		if (
			(binding.action || binding.actions[0]) === input.action &&
			binding.times <= input.times
		) {
			if (!input.value) {
			...
			if (binding.precise) {
				...
				if (
					(Math.sign(binding.value) ||
						binding.values.map((v) => Math.sign(v))) === Math.sign(input.value)
				) {
					boundButtons.push({ button: skyButton, value: !!input.value });
				}
			}
...
function updateBindSettings() {
	...
	if (device)
		buttons.forEach((button) => {
			...
			document.getElementById("setting_" + button + "_bind").disabled =
				!Object.keys(bindings).length ? "true" : null;
